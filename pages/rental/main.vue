<template>
    <view id="rental-main">
        <view class="row-container filter-box">
            ...
            <view id="price-filter-item" class="row-container filter-item" @click="popFilter('price')">
                ...
                <view class="filter-value"
                    >{{ this.filter.priceLimit == 5000 ? '无上限' : '0 - ' + this.filter.priceLimit }}
                </view>
            </view>
        ...
        <scroll-view
            ...
            class="column-container rental-scroll"
            ...
            <view class="column-container filter-popup">
                ...
                <view class="row-container price-filter-pop">
                    <view class="price-slider">
                        <slider
                            activeColor="#9b0000"
                            :value="this.filter.priceLimit"
                            min="0"
                            max="5000"
                            step="100"
                            block-size="15"
                            @change="priceLimitChange"
                            @changing="priceLimitChange"
                        />
                    </view>
                ...
                <view class="floorplan-filter-pop">
                    <view class="floorplan-tag" v-for="(floorplan, index) in floorplanList" :key="index">
                        <uni-tag
                            v-show="this.selectedFloorplan.indexOf(floorplan) == -1"
                            :text="floorplan"
                            @click="select(floorplan)"
                        ></uni-tag>
                        <uni-tag
                            v-show="this.selectedFloorplan.indexOf(floorplan) != -1"
                            custom-style="background-color: #9b0000; border-color: #9b0000; color: #fff;"
                            :text="floorplan"
                            @click="remove(floorplan)"
                        ></uni-tag>
                    </view>
                    <view
                        class="all-floorplan-tag"
                        v-show="
                            this.selectedFloorplan.length == this.floorplanList.length || this.selectedFloorplan.length == 0
                        "
                        @click="clickAll"
                    >
    ...
    methods: {
        ...
        getRentalList: async function () {
            ...
            if (this.filter.time[0] != -1) {
                temp = [
                    moment(this.filter.time[0], 'YYYY-MM-DD').valueOf(),
                    moment(this.filter.time[1], 'YYYY-MM-DD').valueOf(),
                ];
            }
            const opts = {
