		...
		methods:{
			...
			async confirm(){
				if(this.regex.test(this.email)){
					const opts = {
					    path: `/user/updateProfile?str=${encodeURI(this.email)}&subscribe=${this.subscribe}&service=email`,
					...
					requestAPI(opts)
					    .then(response => {
					        if(response.data.status == 100){
					            ...
					            console.error("Profile update failed with status:", response.data.status);
					        }
					    })
					    .catch(error => {
