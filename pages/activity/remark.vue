<template>
    <view id="remark">
        <view class="swiper-indicator">
            <view
                v-for="(info, index) in actDetail.additionalInfo.questions"
                :key="index"
                class="swiper-dot"
                :class="{ 'swiper-dot-selected': current == index }"
        ...
        <uni-forms id="activity-form" ref="activity-form" :modelValue="response" :rules="rules" label-position="top">
            <swiper class="swiper-box" @change="onSwipe" :current="current" vertical>
                <swiper-item class="act-segment" v-for="(info, index) in actDetail.additionalInfo.questions" :key="index">
                    <uni-forms-item :label="info.title" :name="info.title">
                        <view
                            class="placeholder"
                            v-if="
                                info.placeholder &&
                                info.placeholder.length != 0 &&
                                info.type !== 'input' &&
                                info.type !== 'textarea'
                            "
                        >
                        ...
                        <uni-data-checkbox
                            mode="button"
                            v-if="info.type === 'single'"
                            v-model="response[info.title]"
                            :localdata="info.localData"
                            @change="onInputConfirm"
                        ></uni-data-checkbox>
                        <uni-data-checkbox
                            mode="button"
                            v-if="info.type === 'multiple'"
                            multiple
                            v-model="response[info.title]"
                            :localdata="info.localData"
                        ></uni-data-checkbox>
                        <uni-file-picker
                            v-model="this.images[info.title]"
                            v-if="info.type === 'upload'"
                            limit="1"
                            fileMediatype="image"
                            :auto-upload="false"
                            :readonly="isEnd"
                            @select="(event) => onSelectImage(event, info.title)"
                        ></uni-file-picker>
                    </uni-forms-item>
                </swiper-item>
            </swiper>
