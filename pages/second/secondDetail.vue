<template>
	<view class="second-detail">
		...
		<view class="product-detail-box">
			...
			<view class="tag-box">
				<view class="tag delivery-tag"><text style="color: white">{{ this.delivery[product.delivery] }}</text>
				</view>
				<view class="tag condition-tag"><text
						style="color: #7f0019">{{ this.condition[product.productCondition] }}</text></view>
			</view>
			<view class="contact-box">
				<view class="avatar-box">
					<img class="avatar" :src="
                            'https://cssa-mini-na.oss-us-west-1.aliyuncs.com/cssa-mini-avatar/' + product.sellerAvatar + '.jpg'
                        " />
				</view>
				<view class="info-box">
				...
				<view class="contact-overlay" v-show="!this.isLogin">
					<button class="login-button" plain="true" @click="getUserProfile">点击登录可查看联系方式</button>
				</view>
			</view>
...
<script>
	export default {
		...
		onShow() {
			...
			if (this.userInfo.savedProductJSON != null && this.userInfo.savedProductJSON.contains(this.product
				.productID)) {
				this.isSaved = true;
			}
		},
