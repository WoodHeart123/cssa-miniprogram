	...
	<view style="background-color: white; height: 100vh; width: 100%;">
		<div class="find-friend-navbar">
			<div v-for="(item, index) in navbarItems" v-bind:key="item.name">
				<navigator v-if="item.name == '课友'" class="find-friend-navbar-item" v-bind:url="`${item.url}`">
					{{ item.name }}
				</navigator>
				<div v-else v-bind:class="navBarItemStyle(index)" @click="changeTab(index)">{{ item.name }}</div>
			</div>
		...
		data() {
			return {
				...
				navbarItems: [
					...
					{ name: '课友', url: './findClassmate' }, { name: '吃饭', url: '' }, { name: '运动', url: '' }, { name: '旅游', url: '' }, { name: '游戏', url: '' }
				],
