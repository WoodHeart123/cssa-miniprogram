    ...
    <view class="comment-box">
        ...
        <view class="comment-head-area row-container">
            ...
            <view class="user-rate-box">
                <view class="row-container rate-sub-box">
                    ...
                    <view
                        ><text class="user-rate-text">{{ comment.difficulty }}</text></view
                    >
                </view>
                <view class="row-container rate-sub-box">
                    ...
                    <view
                        ><text class="user-rate-text">{{ comment.prefer }}</text></view
                    >
                </view>
            </view>
            <view class="comment-time"
                ><text>{{ computeCommentTime }}</text></view
            >
        </view>
        <view class="wrap">
        ...
        <view class="row-container comment-end">
            ...
            <view class="row-container" @click="addZan">
                <text class="iconfont icon" :class="{ 'thumb-liked': this.comment.liked, '.thumb-liked-animated': this.liked }"
                    >&#xe876;</text
                >
...
export default {
    ...
    computed: {
        computeCommentTime() {
            moment.locale('zh-cn');
            return Date.now() - this.comment.commentTime >= 86400000 * 7
                ? moment(this.comment.commentTime).format('MM-DD')
                : moment(this.comment.commentTime).fromNow();
        },
    },
};
